            ...
            >
                ...
                {!Object.keys(expandedItems).some(id => expandedItems[Number(id)]) && (
                    <div className="absolute inset-0 pointer-events-none opacity-20">
                        <div className="w-full h-full bg-gradient-radial from-lime-400/10 via-transparent to-transparent" />
                    </div>
                )}
                    ...
                    {timelineData.map((item, index) => {
                        ...
                        return (
                            <div
                                ...
                                className={`absolute transition-all duration-700 cursor-pointer will-change-transform ${expanded ? 'z-[200]' : ''
                                    }`}
                                style={nodeStyle}
                                onClick={(e) => { e.stopPropagation(); toggleItem(item.id); }}
                            >
                                {pulsing && (
                                    <div className="absolute rounded-full -inset-2 bg-gradient-radial from-lime-400/25 to-transparent animate-pulse" />
                                )}

                                <div
                                    className={`rounded-full flex items-center justify-center border-2 transition-all duration-500 transform ${expanded
                                            ? "bg-white text-slate-900 border-white shadow-lg shadow-lime-400/30 scale-[1.4]"
                                            : related
                                                ? "bg-white/60 text-slate-900 border-white scale-110"
                                                : "bg-card text-white border-white/40 hover:bg-border/30 hover:scale-105"
                                        }`}
                                    style={{
                                ...
                                <div className={`absolute whitespace-nowrap text-xs font-semibold tracking-wide transition-all duration-500 ${expanded ? "text-white scale-125" : "text-white/85"
                                    }`}
                                    style={{
                                ...
                                {expanded && !isMobile && windowSize.width < 1024 && (
                                    ...
                                    >
                                        ...
                                        <CardHeader className="pb-4">
                                            <CardTitle className="text-base lg:text-lg text-white">{item.title}</CardTitle>
                                        </CardHeader>
                        ...
                        {timelineData
                            ...
                            .map(item => (
                                ...
                                >
                                    ...
                                    <CardContent className="text-xs text-gray-300 px-3">
                                        {item.image && (
                                            <div className="relative aspect-[16/10] overflow-hidden rounded-lg mb-4">
                                                <img
                                                    src={item.image}
