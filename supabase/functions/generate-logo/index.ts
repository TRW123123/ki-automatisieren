...
serve(async (req) => {
    ...
    try {
        let { company_name, business_description, design_goal, logo_style, insert_id, website_url, email } = await req.json();
...
`;
        ...
        if (email) {
            console.log(`Firing n8n webhook to deliver logo to ${email}...`);
